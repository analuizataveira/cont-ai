...
const envFile = process.env.NODE_ENV === "test" || process.env.NODE_ENV === "dev" ? ".env.test" : ".env";
config({ path: envFile });
...
export const AppDataSource = new DataSource({
  ...
  username: process.env.DB_USERNAME ?? process.env.DB_TEST_USERNAME ?? "postgres",
  password: process.env.DB_PASSWORD ?? process.env.DB_TEST_PASSWORD ?? "postgres",
  database: process.env.DB_NAME ?? process.env.DB_TEST_NAME ?? "contai",
